...
#### カスタムアプリが Shopify 管理画面に表示される仕組み

カスタムアプリは **Shopify 管理画面に埋め込まれて表示**されます。別タブで開くのではなく、管理画面のレイアウト内に組み込まれます。

1. **アプリのインストール**: ストアオーナーがアプリをインストールし、OAuth で認証が完了する。
2. **管理画面からの起動**: Shopify 管理画面の左サイドバー「アプリ」メニューにアプリが表示され、クリックで起動する。
3. **埋め込み表示（iframe）**: アプリの URL が **iframe** 内で読み込まれる。ユーザーには「Shopify 管理画面の一部」のように見え、左サイドバーやヘッダーは Shopify のまま、中央のコンテンツ部分がアプリの画面になる。
4. **App Bridge**: Shopify が提供する JS ライブラリ。iframe 内のアプリが親の Shopify 管理画面とやり取りする（トースト表示、ナビゲーション、認証情報の取得など）。
5. **ホスティング**: アプリ本体は**自前サーバー**で稼働。Shopify はその URL を iframe の `src` として読み込むだけ。

[Shopify App Template (Remix)](https://github.com/Shopify/shopify-app-template-remix) を使うと、埋め込み用レイアウト・OAuth・Polaris（Shopify の UI コンポーネント）が最初から用意され、オファー作成画面などのビジネスロジックに集中できます。
